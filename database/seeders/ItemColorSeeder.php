...
class ItemColorSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        foreach ($items as $item) {
            $productName = str_replace(' ', '_', $item->product_name); // Replace spaces with underscores for the image name
            ...
            $colorImage3Url = asset('images/product_images/' . str_replace(' ', '_', $productName) . '_color_3.jpg'); //Example: product_images/Product_A_color_3.jpg
            ...
            while (count($colorImages) < count($itemColorMap)) {
                $colorImages[] = $colorImages[0]; // Or use a placeholder/default image
            }

            if (!isset($itemColorMap[$item->product_name])) {
                continue; // skip if no color mapping defined for this item
            }
            ...
            foreach ($colors as $index => $color) {
                ...
                ItemColor::create([
                    'item_id' => $item->id,
                    'name' => $color['name'],
                    'image_path' => $colorImages[$imageIndex],
