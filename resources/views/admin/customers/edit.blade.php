    ...
    <form class="max-w-sm mx-auto" action="{{ route('admin.customers.update', $customer->id) }}" method="POST">
        ...
        <div class="mb-6">
            ...
            <select id="city" name="city"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                <option value="addisababa" {{ old('city', $customer->city) == 'addisababa' ? 'selected' : '' }}>Addis
                    Ababa</option>
                <option value="adama" {{ old('city', $customer->city) == 'adama' ? 'selected' : '' }}>Adama</option>
                <option value="direDawa" {{ old('city', $customer->city) == 'direDawa' ? 'selected' : '' }}>Dire Dawa
                </option>
                <option value="bahirDar" {{ old('city', $customer->city) == 'bahirDar' ? 'selected' : '' }}>Bahir Dar
                </option>
                <option value="bishoftu" {{ old('city', $customer->city) == 'bishoftu' ? 'selected' : '' }}>Bishoftu
                </option>
                ...
                <option value="shashamanea" {{ old('city', $customer->city) == 'shashamanea' ? 'selected' : '' }}>
                    Shashamanea</option>
            </select>
            @error('city')
                <div class="text-red-500 text-sm">{{ $message }}</div>
        ...
        <div class="mb-4">
            <button type="submit"
                class="w-full bg-indigo-600 text-white p-3 rounded-lg hover:bg-indigo-700 focus:ring-4 focus:ring-indigo-500">Update</button>
        </div>
    </form>
