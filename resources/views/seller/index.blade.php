{{--
  @php
  // Get the theme from the session, default to 'admintheme' if not set
  $theme = session('theme', 'sellerandstock_keepertheme');
  @endphp
--}}
...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
  ...
  <body
    class="bg-{{ session('theme', 'sellerandstock_keepertheme') }}-base-100 text-{{ session('theme', 'sellerandstock_keepertheme') }}-primary"
  >
    <main>
      {{-- <nav class="bg-{{ $theme }} dark:bg-{{ $theme }} antialiased"> --}}
      <nav
        class="bg-{{ session('theme', 'sellerandstock_keepertheme') }} dark:bg-{{ session('theme', 'sellerandstock_keepertheme') }}-dark antialiased"
      >
